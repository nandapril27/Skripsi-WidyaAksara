...
class NilaiMenulisNgalagenaActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_nilai_menulis_ngalagena)
    ...
    private fun fetchNilai(nis: String) {
        ApiClient.instance.getNilaiKuisMenulisNgalagena(nis).enqueue(object : Callback<NilaiResponse> {
            override fun onResponse(call: Call<NilaiResponse>, response: Response<NilaiResponse>) {
                if (response.isSuccessful && response.body() != null) {
                    ...
                    if (nilaiList.isNotEmpty()) {
                        ...
                        Toast.makeText(this@NilaiMenulisNgalagenaActivity, "Tidak ada data nilai", Toast.LENGTH_SHORT).show()
                    }
                } else {
                    Log.e("API Error", "Gagal mengambil data. Response code: ${response.code()}")
                    Log.e("API Error", "Response body: ${response.errorBody()?.string()}")

                    Toast.makeText(this@NilaiMenulisNgalagenaActivity, "Gagal mengambil data nilai", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<NilaiResponse>, t: Throwable) {
                Log.e("API Failure", "Terjadi kesalahan: ${t.message}")
                Toast.makeText(this@NilaiMenulisNgalagenaActivity, "Terjadi kesalahan jaringan", Toast.LENGTH_SHORT).show()
            }
        })
    }
}
